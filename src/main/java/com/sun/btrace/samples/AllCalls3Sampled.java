 ...
 *
 * Not all instances of the method call are intercepted, however. Adaptive
 * sampling is used to balance the captured data and incurred overhead.
 */
@BTrace
public class AllCalls3Sampled {
    @OnMethod(clazz = "javax.swing.JButton", method = "/.*/", location = @Location(value = Kind.CALL, clazz = "/.*/", method = "/.*/"))
    @Sampled(kind = Sampled.Sampler.Adaptive)
    public static void o(@Self Object self, @ProbeMethodName String pmn,
            AnyType[] args) { // all calls to methods
