 ...
 *
 * Not all instances of the method call are intercepted, however. Sampling is
 * used to pick only statistically representative ones.
 */
@BTrace
public class AllCalls2Sampled {
    @OnMethod(clazz = "/javax\\.swing\\..*/", method = "/.*/", location = @Location(value = Kind.CALL, clazz = "/.*/", method = "/.*/"))
    @Sampled
    public static void n(@Self Object self, @ProbeClassName String pcm,
            @ProbeMethodName String pmn, @TargetInstance Object instance,
            @TargetMethodOrField String method, String text) { // all calls to
                                                               // the methods
                                                               // with signature
                                                               // "(String)"
