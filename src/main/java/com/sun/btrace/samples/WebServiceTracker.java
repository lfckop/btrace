...
/**
 * A simple BTrace program that prints a class name and method name whenever a
 * webservice is called and also prints time taken by service method. WebService
 * entry points are annotated javax.jws.WebService and javax.jws.WebMethod. We
 * insert tracing actions into every class and method annotated by these
 * annotations. This way we don't need to know actual webservice implementor
 * class name.
 */
@BTrace
public class WebServiceTracker {
    ...
    @OnMethod(clazz = "@javax.jws.WebService", method = "@javax.jws.WebMethod", location = @Location(Kind.RETURN))
